/**
 * @file conv1d_omp.c
 * @brief OpenMP-enabled 1-D Convolution with enhanced kernels, smarter chunking, GFLOPS metrics,
 ...
 *        - GFLOPS report: 2*N*K / time (approx; SAME ignores trimmed edges).
 *
 * Build:
 *   cc -std=c11 -O3 -march=native -Wall -Wextra -Werror -fopenmp -o conv1d_omp conv1d_omp.c
 *
 * Example:
 ...
 *   • FULL mode (enhanced) also parallelizes over output index n, removing atomic updates.
 ...
 * and output path. Also supports --len/-L and --klen/-kL for generated inputs.
 *
 * @param argc process argc
 * @param argv process argv
 * @param f_path [out] path to f (or NULL for RNG)
 * @param g_path [out] path to g (or NULL for RNG)
 * @param o_path [out] path to output file (required)
 * @param N_req [out] requested length of f if RNG (-L/--len)
 * @param K_req [out] requested length of g if RNG (-kL/--klen)
 * @param seed  [out] RNG seed (defaults to time(NULL))
 * @param have_seed [out] 1 if user provided seed
 * @param cmode [out] convolution mode (same/full)
 * @param pmode [out] padding mode for SAME
 * @param cval  [out] constant pad value for PAD_CONST
 * @param have_cval [out] 1 if user provided cval
 * @param threads [out] requested threads
 * @param have_threads [out] 1 if user provided threads
 * @param sched_kind [out] OpenMP schedule kind
 * @param have_sched [out] 1 if user provided schedule
 * @param chunk [out] schedule chunk size
 * @param have_chunk [out] 1 if user provided chunk
               ...
               int *chunk, int *have_chunk)
{
    ...
    *have_chunk = 0;
    ...
    /* Pre-scan for -L / -kL so getopt order doesn't matter */
...
/**
 * @brief 1-D convolution in FULL mode with OpenMP (enhanced: no atomics).
 *
 * Parallelizes over output element n in [0..N+K-2]. For each n, determines the
 * valid overlap of f[i] and g[j] (j=n-i), and accumulates into a private sum.
 * Inner loop uses #pragma omp simd reduction to improve vectorization.
 *
 * @param f input signal of length N
 * @param N length of input signal
 * @param g kernel of length K
 * @param K length of kernel
 * @param out output buffer of length N+K-1 (pre-allocated)
 */
void conv1d_full_omp(const float *__restrict f, int N,
                     const float *__restrict g, int K,
                     float *__restrict out)
...
#pragma omp parallel for schedule(runtime)
    ...
    {
        /* valid i overlap: i in [max(0,n-(K-1)) .. min(n,N-1)], j = n - i */
        int i0 = n - (K - 1);
        if (i0 < 0)
            i0 = 0;
        int i1 = (n < N - 1) ? n : (N - 1);
...
/**
 * @brief 1-D convolution in SAME mode with OpenMP (enhanced: trimmed bounds + SIMD).
 *
 * Parallelizes over output element n in [0..N-1]. Computes tight bounds [m0..m1]
 * such that idx = n + (m - c) is in [0..N-1] with c=K/2, removing branches in
 * the inner loop. PAD_CONST handled by adding cval * sum of g outside [m0..m1].
 *
 * @param f input signal of length N
 * @param N length of input signal
 * @param g kernel of length K
 * @param K length of kernel
 * @param out output buffer of length N (pre-allocated)
...
#pragma omp simd reduction(+ : acc)
        for (int m = m0; m <= m1; m++)
        {
            int idx = n + (m - c); /* in-bounds by construction */
            acc += (double)f[idx] * (double)g[m];
        }

        if (pmod == PAD_CONST)
        {
            /* head [0..m0-1] and tail [m1+1..K-1] contribute cval*g[m] */
            double pad_sum = 0.0;
            for (int m = 0; m < m0; m++)
                pad_sum += (double)g[m];
 ...
 *
 * Fields: RunID,N,K,outLen,mode,padding,cval,time_s,gflops,threads,schedule,chunk
 *
 * @param N length of f
 * @param K length of g
                 ...
                 int threads, const char *sched_str, int chunk)
{
    ...
    fprintf(csv, "RunID,N,K,outLen,mode,padding,cval,time_s,gflops,threads,schedule,chunk\n");
    fprintf(csv, "%s,%d,%d,%d,%s,%s,%.9g,%.9f,%.6f,%d,%s,%d\n",
            runid, N, K, outLen,
            (cmode == MODE_FULL ? "full" : "same"),
            (pmode == PAD_ZERO ? "zero" : (pmode == PAD_NONE ? "none" : "const")),
            (pmode == PAD_CONST ? (double)cval : 0.0),
            elapsed_secs, gflops, threads, sched_str, chunk);
...
int main(int argc, char **argv)
{
    ...
    if (have_seed)
        ...
        srand((unsigned)seed); /* same behavior; documented value */
    ...
    /* Smarter chunk default if user didn't provide one:
       work = N (same) or (N+K-1) (full); aim for ~4 chunks per thread. */
    if (!have_chunk)
    {
...
#pragma omp parallel for schedule(static)
    ...
    fprintf(stderr,
            "N=%d K=%d outLen=%d mode=%s pad=%s cval=%.6g | conv_time=%.9f s | %.3f GFLOP/s | OMP{threads=%d sched=%s chunk=%d}\n",
            N, K, outLen,
            (cmode == MODE_FULL ? "full" : "same"),
            (pmode == PAD_ZERO ? "zero" : (pmode == PAD_NONE ? "none" : "const")),
    ...
    log_metrics(N, K, outLen, cmode, pmode, cval, secs, gflops, threads, sched_str, chunk);
