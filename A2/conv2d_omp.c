               ...
               int *chunk, int *have_chunk)
{
    ...
    *have_chunk = 0;
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        /* Handle -sH and -sW stride parameters */
        if (!strcmp(a, "-sH"))
        {
            if (i + 1 >= argc)
            {
...
/* ------------------------- Correlation kernels ------------------------- */
/**
 * @brief 2-D FULL cross-correlation with stride (no flip inside).
 * @param F   H×W image.
 * @param H,W Dimensions of F.
 * @param G   KH×KW kernel.
 * @param KH,KW Kernel dims.
 * @param sH,sW Stride parameters (vertical, horizontal).
 * @param Y   Output ceil((H+KH-1)/sH) × ceil((W+KW-1)/sW).
 */
void corr2d_full_stride(const float *F, int H, int W,
                        const float *G, int KH, int KW,
                        int sH, int sW,
...
/**
 * @brief 2-D SAME cross-correlation with stride and anchor control (no flip inside).
 *
 * Mathematical form (no kernel flip):
 *   Y[i_out, j_out] = sum_{u=0..KH-1} sum_{v=0..KW-1} F[i + (u - cH), j + (v - cW)] * G[u,v]
 *   where i = i_out * sH, j = j_out * sW.
 *
 ...
 * @param KH,KW Kernel dims.
 * @param sH,sW Stride parameters (vertical, horizontal).
 * @param Y   Output ceil(H/sH) × ceil(W/sW).
 * @param pmod Padding policy (zero/none/const).
 * @param cval Constant pad value (if PAD_CONST).
 * @param use_conv_anchor 1 for convolution anchor (KH/2), 0 for correlation anchor ((KH-1)/2).
 */
void corr2d_same_stride(const float *F, int H, int W,
                        const float *G, int KH, int KW,
                        int sH, int sW,
...
#pragma omp simd reduction(+ : acc)
            ...
            if (pmod == PAD_CONST)
            {
                ...
                /* Top and bottom out-of-bounds rows */
                for (int u = 0; u < u0; u++)
                    for (int v = 0; v < KW; v++)
                ...
                /* Left and right out-of-bounds columns in valid rows */
                for (int u = u0; u <= u1; u++)
                {
                    for (int v = 0; v < v0; v++)
                        pad_sum += (double)G[IDX2(u, v, KW)];
                 ...
                 int threads, const char *sched_str, int chunk)
{
    ...
    fprintf(csv, "RunID,H,W,KH,KW,outH,outW,op,mode,padding,cval,sH,sW,time_s,gflops,threads,schedule,chunk\n");
    fprintf(csv, "%s,%d,%d,%d,%d,%d,%d,%s,%s,%s,%.6g,%d,%d,%.9f,%.6f,%d,%s,%d\n",
...
int main(int argc, char **argv)
{
    ...
    /* Output allocation with stride-aware dimensions */
...
#pragma omp parallel for schedule(static)
    ...
    fprintf(stderr,
            "op=%s H=%d W=%d KH=%d KW=%d outH=%d outW=%d mode=%s pad=%s cval=%.6g stride=%dx%d | "
    ...
    /* Metrics CSV */
    log_metrics(H, W, KH, KW, outH, outW, op, cmode, pmode, cval, sH, sW, secs, gflops, threads, sched_str, chunk);
