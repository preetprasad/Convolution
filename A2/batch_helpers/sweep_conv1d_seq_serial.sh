#!/usr/bin/env bash
# Serial SLURM sweep for conv1d (sequential)
# Submits one job at a time and blocks until BOTH stderr and metrics CSV appear.
#
# Usage:
#   ./sweep_conv1d_seq_serial.sh NMIN NMAX N_STEP KMIN KMAX K_STEP [STRIDE] [POST_COPY_WAIT] [FILE_WAIT_RETRIES] [SEED]
#
# Positional:
...
if [[ $# -lt 6 ]]; then
  echo "Usage: $0 NMIN NMAX N_STEP KMIN KMAX K_STEP [STRIDE] [POST_COPY_WAIT] [FILE_WAIT_RETRIES] [SEED]" >&2
  exit 1
fi

NMIN="$1"; NMAX="$2"; NSTEP="$3"
KMIN="$4"; KMAX="$5"; KSTEP="$6"
STRIDE="${7:-1}"              # output stride
POST_COPY_WAIT="${8:-10}"     # seconds
FILE_WAIT_RETRIES="${9:-60}"  # each retry waits 2s
SEED="${10:-}"                # optional

# Validation
for v in "$NMIN" "$NMAX" "$NSTEP" "$KMIN" "$KMAX" "$KSTEP" "$POST_COPY_WAIT" "$FILE_WAIT_RETRIES"; do
...
echo "  STRIDE=$STRIDE  POST_COPY_WAIT=${POST_COPY_WAIT}s  FILE_WAIT_RETRIES=$FILE_WAIT_RETRIES  SEED=${SEED:-<default>}"
...
submit_and_block() {
  ...
  if [[ -n "$SEED" ]]; then
    submit_out=$(sbatch slurm_helpers/conv1d_seq_param.slurm "$N" "$K" same zero "$STRIDE" "$SEED")
  else
    submit_out=$(sbatch slurm_helpers/conv1d_seq_param.slurm "$N" "$K" same zero "$STRIDE")
  fi
