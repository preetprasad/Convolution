...
#
# Usage:
#   ./sweep_conv2d_mpi_serial.sh HMIN HMAX H_STEP WMIN WMAX W_STEP KHMIN KHMAX KH_STEP KWMIN KWMAX KW_STEP [POST_COPY_WAIT] [FILE_WAIT_RETRIES] [NP] [SEED]
#
# Positional:
...
if [[ $# -lt 12 ]]; then
  echo "Usage: $0 HMIN HMAX H_STEP WMIN WMAX W_STEP KHMIN KHMAX KH_STEP KWMIN KWMAX KW_STEP [POST_COPY_WAIT] [FILE_WAIT_RETRIES] [NP] [SEED]" >&2
  exit 1
fi
...
submit_and_block() {
  ...
  submit_out=$(sbatch slurm_helpers/conv2d_mpi_param.slurm "$H" "$W" "$KH" "$KW" "$NP" same zero "$SEED")
