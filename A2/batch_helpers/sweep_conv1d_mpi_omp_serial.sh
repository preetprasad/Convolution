...
#
# Usage:
#   ./sweep_conv1d_mpi_omp_serial.sh LMIN LMAX L_STEP KMIN KMAX K_STEP [POST_COPY_WAIT] [FILE_WAIT_RETRIES] [NP] [THREADS] [SCHED] [CHUNK] [SEED]
#
# Positional:
...
if [[ $# -lt 6 ]]; then
  echo "Usage: $0 LMIN LMAX L_STEP KMIN KMAX K_STEP [POST_COPY_WAIT] [FILE_WAIT_RETRIES] [NP] [THREADS] [SCHED] [CHUNK] [SEED]" >&2
  exit 1
fi
...
submit_and_block() {
  ...
  submit_out=$(sbatch slurm_helpers/conv1d_mpi_omp_param.slurm "$L" "$K" "$NP" "$THREADS" "$SCHED" "$CHUNK" same zero "$SEED")
