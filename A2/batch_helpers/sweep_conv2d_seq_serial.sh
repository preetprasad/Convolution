#!/usr/bin/env bash
# Serial SLURM sweep for conv2d (sequential)
# Submits one job at a time and blocks until BOTH stderr and metrics CSV appear.
#
# Usage:
#   ./sweep_conv2d_seq_serial.sh HMIN HMAX H_STEP WMIN WMAX W_STEP KHMIN KHMAX KH_STEP KWMIN KWMAX KW_STEP [STRIDE_H] [STRIDE_W] [POST_COPY_WAIT] [FILE_WAIT_RETRIES] [SEED]
#
# Positional:
...
if [[ $# -lt 12 ]]; then
  echo "Usage: $0 HMIN HMAX H_STEP WMIN WMAX W_STEP KHMIN KHMAX KH_STEP KWMIN KWMAX KW_STEP [STRIDE_H] [STRIDE_W] [POST_COPY_WAIT] [FILE_WAIT_RETRIES] [SEED]" >&2
  exit 1
fi
...
STRIDE_H="${13:-1}"            # vertical stride
STRIDE_W="${14:-1}"            # horizontal stride
POST_COPY_WAIT="${15:-10}"     # seconds
FILE_WAIT_RETRIES="${16:-60}"  # each retry waits 2s
SEED="${17:-}"                 # optional
...
echo "  STRIDE_H=$STRIDE_H  STRIDE_W=$STRIDE_W"
...
submit_and_block() {
  ...
  if [[ -n "$SEED" ]]; then
    submit_out=$(sbatch slurm_helpers/conv2d_seq_param.slurm "$H" "$W" "$KH" "$KW" same zero "$STRIDE_H" "$STRIDE_W" "$SEED")
  else
    submit_out=$(sbatch slurm_helpers/conv2d_seq_param.slurm "$H" "$W" "$KH" "$KW" same zero "$STRIDE_H" "$STRIDE_W")
  fi
